...
The default is **the approved Sprint 1 authoring workflow and presentation**, including concise prose, consolidated instructions, purposeful visuals, and adjacent responses. Read the [presentation reference and approved examples](docs/AUTHORING_PRESENTATION.md) before substantive drafting. Use its local preview/check workflow and record the actual desktop/mobile review; the supplied design still determines learning, grading, and task structure.
...
Use `[label](artifact:<artifact_id>)` for links between participant-facing course artifacts. The renderer resolves the default link to the public hosted page and records the selected profile's Canvas item URL separately. When the page runs inside Canvas, the browser activates the Canvas destination. Unknown or non-renderable artifact IDs fail validation. Do not put Canvas course IDs, page URLs, assignment IDs, discussion IDs, or quiz IDs in participant-facing Markdown.

Hosted assignment, discussion, and quiz pages show their complete directions publicly, but submission controls are Canvas-only. Public visitors see the submission explanation without a link that triggers an institutional login. Inside Canvas, the renderer activates the submission link from deployment state. AI-activity shells follow the same rule.

Hosted course homepages are regenerated at render time from artifact frontmatter and deployment state. The renderer writes `<path_prefix>/<course>/home.html`, `index.html`, `sprint-<n>.html`, and `progress-map.json` for each hosted course. When `<course>/homepage.yaml` exists, it is curated display metadata only: course lead text, module goals, group labels, item meta, item icons, web-context links, badges, and verification notes. New artifacts and new sprints still appear automatically from Markdown even when the YAML has not been curated yet. Every default link remains on the public hosted course. Canvas-context links come from deployment state and activate only for `?context=canvas` or iframe use.
