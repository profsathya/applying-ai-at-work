...
def table_matrix(block: dict, view: str = 'base', *, reviewed_structure: bool = False) -> list[list[str]]:
    ...
    if len(matrix) < 2 or not matrix[0] or any(len(row) != len(matrix[0]) for row in matrix):
        raise TableMappingError('Table needs a header and equally wide body rows')
    if any(not heading.strip() for heading in matrix[0]):
...
def create_task(block: dict, task_id: str, prompt: str, criteria: list[str] | None = None, *, view: str = 'base',
                ...
                reviewed_structure: bool = False, read_only: bool = False) -> dict:
    matrix = table_matrix(block, view, reviewed_structure=reviewed_structure)
    if not re.fullmatch(r'[a-z][a-z0-9-]*', task_id):
        raise TableMappingError('Task ID must be lowercase kebab-case')
    chosen = response_coordinates(matrix, response_rows or set(), response_cells or set(), read_only=read_only)
    ...
    task = {'id': task_id, 'kind': 'table', 'prompt': prompt,
            'columns': [{'id': f'column-{i}', 'label': heading}
                        for i, heading in enumerate(matrix[0], 1)], 'rows': rows}
    if read_only:
        task['read_only'] = True
    else:
...
def response_coordinates(matrix: list[list[str]], response_rows: set[int],
                         response_cells: set[tuple[int, int]], *, read_only: bool = False) -> set[tuple[int, int]]:
    ...
    if read_only:
        ...
        return set()
    if not chosen or any(row < 2 or row > len(matrix) or column < 1 or column > len(matrix[0])
...
def compare_task(block: dict, task: dict, *, view: str = 'base',
                 ...
                 reviewed_structure: bool = False, read_only: bool | None = None) -> list[str]:
    matrix = table_matrix(block, view, reviewed_structure=reviewed_structure)
    errors = []
    ...
    if [column.get('label') for column in task.get('columns', [])] != matrix[0]:
    ...
    for index, (row, source_row) in enumerate(zip(rows, matrix[1:]), 1):
    ...
    if response_cells is not None:
        actual = {(row_index, column_index) for row_index, row in enumerate(rows, 2)
...
def main() -> int:
    ...
    try:
        ...
        chosen = response_coordinates(table_matrix(block, args.view, reviewed_structure=args.reviewed_structure),
                                      set(args.response_row), cells, read_only=args.read_only)
        if args.artifact:
