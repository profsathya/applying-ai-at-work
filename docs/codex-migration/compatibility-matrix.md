...
| Mechanical pusher | `.claude/agents/canvas-pusher.md` | Runs `push.py` | Unnecessary LLM wrapper plus Claude permissions | Direct script call gated by explicit approval | low | high | Repo fact: `push.py` owns Canvas side effects and deployment-state updates. |
