...
Provisioning status (September 16, 2026): the [work-owned course document](https://docs.google.com/document/d/1q4Dpbzr1quGMw-_i5mn99EO4bi-xj-VK-DoAJG13r38/edit) belongs to `jeshaw@csumb.edu` and preserves the Dojo and Course tabs from the verified initial snapshot. It is private while participant viewer access is pending. The initial snapshot contains the canonical Core and 25 verified published artifacts. This initial fill was copied from the verified personal-account staging document, not generated by the automatic workflow. The participant setup page explicitly says automatic refresh is not enabled.

The [work-owned Apps Script project](https://script.google.com/home/projects/1WVriQ1-dM-QGOD-Zoj8KoXQxj-G5VnkjDmKU2a1CXMc6kbbsvmORtuWQ/edit) belongs to `jeshaw@csumb.edu` and has the repository receiver source saved. Its document mapping, shared-token property, authorization, and web-app deployment are still pending. The receiver must execute under this work account so its document edits do not depend on the personal account.
