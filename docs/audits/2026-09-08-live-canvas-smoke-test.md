...
## Follow-up: course homepage addition

The user subsequently requested access through the existing course homepage. Inspection confirmed Canvas front page 3467 (`home`) embeds external tool 203, which launches the hosted `deanza/course1/home.html`. The three shared outputs still lacked the smoke test, although its source metadata and standalone sprint page already existed.

Set only Sprint 10's homepage `open` metadata to `true`, then used the deterministic shared-output renderer with the current authoritative state. Published only `deanza/course1/home.html`, its identical `index.html` alias, and `progress-map.json` in [Common Curriculum commit 970a0ba](https://github.com/profsathya/Common-Curriculum/commit/970a0baa180f4b40f7c2defc70b1403fffad1b49). Removing the appended smoke-test block from either generated HTML file reproduces the previous deployed file exactly. All 69 previous progress records are unchanged; the only additions map existing items 17958, 17959, and 17960 with `must_view`, `must_submit`, and `must_submit` respectively. No unrelated draft was exposed, no artifact body was regenerated, and no Canvas write was required.

Open the [actual Canvas course homepage](https://cti-courses.instructure.com/courses/180) or the [hosted homepage](https://profsathya.github.io/Common-Curriculum/deanza/course1/home.html). The new expanded section at the bottom is titled **Course Builder Smoke Test - 2026-09-08**. Its three links route to native Canvas content in Canvas context and the existing hosted instructions in web context. Source metadata was committed on the audit branch as `6202496`; external deployment-state mappings remain unchanged at `f88db58`.

Homepage and full schema validation passed. [Pages deployment 34282241800](https://github.com/profsathya/Common-Curriculum/actions/runs/34282241800) succeeded; the accompanying context-doc workflow also succeeded and skipped document writes. All three canonical shared-output URLs returned HTTP 200 with hashes identical to the generated files. The three hosted activity links returned HTTP 200, and the Canvas destinations resolved to the existing page, assignment, and quiz. API readback confirmed the original Canvas front-page body and settings were unchanged. Generated diff and final HTTP/API evidence are retained under `course1/reports/smoke-20260908/homepage-*`.

Visual verification also passed in the user's authenticated Canvas browser tab at `/courses/180`. After reload, the existing LTI embed displayed the expanded test section, all three correctly labeled native Canvas links, and completion indicators loaded from the existing progress service. Scrolled the embedded homepage to the section and left it visible for inspection. Hosted-context link destinations were checked through generated HTML and live HTTP readback; no learner attempt or submission was made. This follow-up verifies homepage display and progress readback in that browser session, without extending the earlier test to mobile UX, grading, or AI execution.
