...
`Open orientation` or `Open Sprint N` opens the corresponding native Canvas module. Beneath the card, each available module title also opens its own Canvas module directly. Module destinations are derived from the deployment-state IDs of the curated activities, so old versions in deployment history cannot redirect the current module. Native links target the top-level Canvas window and never carry the hosted progress token. An unbound local preview without deployment IDs retains a generated-module fallback. A separate arrow button reveals or hides that module's activities without navigating. The featured module is omitted from this lower list. The standalone `modules.html` directory retains all scheduled modules and the same title/arrow controls. Both views use the curated course inventory: the native Canvas Modules page still contains published legacy versions. No legacy publication settings change. Help stays visible on both pages.
