...
An optional top-level `schedule` in `<course>/homepage.yaml` selects the focused current-sprint homepage. Courses without it retain the existing module-list homepage. CTI identity remains in the header and footer. The homepage shows a short current-sprint card and one large button that opens that module directly. The remaining modules appear below the card under Course modules, with their activity lists collapsed. Detailed instructions remain on the activity pages.

`Open orientation` or `Open Sprint N` opens the corresponding public hosted module by default. Beneath the card, each available module title also opens its hosted module. When the homepage is opened with `?context=canvas` or inside an iframe, those links switch to the corresponding native Canvas modules. Canvas destinations are derived from the deployment-state IDs of the curated activities, so old versions in deployment history cannot redirect the current module. Native links target the top-level Canvas window and never carry the hosted progress token. An unbound local preview without deployment IDs keeps the generated hosted destination. A separate arrow button reveals or hides that module's activities without navigating. The featured module is omitted from this lower list. The standalone `modules.html` directory retains all scheduled modules and the same title/arrow controls. Both views use the curated course inventory: the native Canvas Modules page still contains published legacy versions. No legacy publication settings change. Help stays visible on both pages.
...
The schedule changes homepage emphasis only. It does not change assignment due dates, publish Canvas modules, lock future work, mark anything complete, or override individual progress. Available modules remain reachable throughout. Native module navigation returns to the existing Canvas course session. The hosted progress token remains in browser storage and is not added to Canvas module URLs.
...
Render through `canvas_sync.hosted_html.render_hosted_files` with authoritative deployment state. Publish `home.html`, its `index.html` alias, and `modules.html` together. Never hand-edit generated pages. Verify desktop/mobile layout, the large button destination in both web and Canvas contexts, independent module-title links and arrow disclosures, before-start behavior, each date boundary, and unavailable material. Static no-JavaScript fallback keeps every effective link on the public hosted course and hides inactive disclosure buttons.
