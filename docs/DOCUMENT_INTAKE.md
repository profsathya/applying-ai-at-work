...
Select faithful conversion or instructional adaptation from the user request as described in [Authoring and Review](AUTHORING.md#source-conversion-and-instructional-adaptation). Preservation applies to passages classified as source selections; an authorized adaptation can shorten or reorganize prose using labelled new parts with reasons and source references. Do not classify rewritten prose as an exact source selection.
...
For newly authored prose and authorized instructional adaptations, use **the approved Sprint 1 authoring workflow and presentation**. Before drafting, read [the presentation reference](AUTHORING_PRESENTATION.md) and inspect the matching approved examples. Use concise connected prose, consolidated directions, purposeful visuals, and adjacent responses while preserving the source's learning and assessment requirements. Faithful conversion and explicitly protected wording remain governed by the source-authority rules above.
...
Walk-through assignments can map a simple rectangular document table to a `kind: table` task. The private intake packet retains its ordered rows and cells; `canvas_sync/walkthrough_tables.py` drafts the task from a selected block after the author explicitly marks response rows or cells, or passes `--read-only` for a reference table, and can compare the authored task back to that block. The generated task keeps source cell text and blank rows, while participant answers remain separate browser-local values. This path requires a single header row and no merged cells. The resulting page, rich clipboard copy, and Word export use the same grid; optional AI feedback reviews one response row at a time. The existing group `layout: table` is a separate field-per-entry presentation.

This builder now supports `delivery_mode: guided_assignment` with `type: assignment` and `submission_type: text_entry`. Its `guided_assignment` configuration contains a version, optional purpose/prior-work framing, a standing instruction, and ordered tasks. Each task has a stable `id` and `prompt`; editable tasks also require `criteria`, while `kind: table` with `read_only: true` omits them. Response tasks can include a reflection; choice tasks additionally require `kind: choice`, `options`, `correct_index`, and `explanation`. Choice feedback is formative, and keys are present in the delivered client configuration. This mode is unsuitable for a secure exam and does not perform automatic grading.

By default the renderer preserves the full authored Markdown inside an instructions panel and provides task-specific self-check panels, response controls, browser-local saving, copy buttons, and a selectable-text fallback. For an integrated teaching/activity sequence, a task may set `instruction_section` to the exact visible title of a unique top-level `##` Markdown heading (inline emphasis is allowed). That section, through the next level-two heading, appears once immediately before the task. Each section can be referenced by only one task. Referenced sections follow task order; unreferenced content appears visibly before the tasks. Missing or ambiguous references fail artifact validation. Keep essential teaching in these sections, prompts focused on the requested action, and criteria focused on evidence. Activities with no references keep their existing layout. Adding references does not require a configuration-version change or reset browser drafts. Saving and copying never claim Canvas submission. Local storage is scoped to artifact identity and configuration version. Changing the version starts a separate draft; it does not delete old storage. Feedback is visibly marked when it applies to an earlier answer. Copying answers includes the participant's current responses, not hidden coach notes or AI feedback.

Choose the continuous-page layout from [the presentation reference](AUTHORING_PRESENTATION.md). `guided_assignment.presentation: reading` supports one or more response/choice tasks without an AI feedback endpoint and avoids generated opening framing. `compact` supports exactly one response task with an `instruction_section` and no feedback endpoint, as in the approved source-derived first activity; without source provenance, its outer renderer can still add a generic goal panel. Do not invent provenance to suppress that panel. In both layouts, the body owns essential directions, standing instructions appear beside submission, and secondary copy/clear/manual-copy tools use a native More options disclosure; a clipboard failure opens and focuses manual copying. Task identity and configuration version still determine saved-draft continuity. Omission or `standard` preserves the existing layout.
