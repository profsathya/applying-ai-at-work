...
For a continuous single-response page, opt into `guided_assignment.presentation: compact`. This requires exactly one response task with an `instruction_section`, and no feedback endpoint. The body owns the opening and essential directions; purpose/prerequisite metadata is retained without separate display. Standing instructions appear beside submission. Secondary copy/clear/manual-copy tools use a native More options disclosure; a clipboard failure opens and focuses manual copying. Task identity and configuration version still determine saved-draft continuity. Omission or `standard` preserves the existing layout.
