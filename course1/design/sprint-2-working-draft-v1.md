...
storage: course1/sprints/sprint-16, positions 1 to 12; published 24 September (Jeremy), walkthroughs released the same day; the built files are edited directly from here on, not regenerated
---
...
| 11 | Problem Frame: taking stock of where you are | Graded item | 45 | one submission | `reading`, 3 boxes | Two |
...
*Own your progress. Position 3. 0 points. No AI on this one. Live as "Dig into your assumptions Canvas Walkthrough" (Jeremy's release, 24 September): the learner downloads the Word file and uploads it in Canvas; the reading version sits unpublished behind it.*
...
[NOTE] On the live walkthrough this line reads (Jeremy, 24 September): "When you finish, select Download as Word document below. In Canvas, select Start Assignment, attach that Word file, and select Submit Assignment. Keep the file for the Dojo Lab, where you will test these assumptions. Downloading it alone does not submit your work." The working-book template line is gone from the walk page; the working book is where the Dojo Lab and the write-up still point. Course-wide list item on the working book.
...
> **Example:** All four managers are affected by account handovers not happening, not just the one I sat next to.

You don't know if this is true. You're assuming it. Behind that assumption are questions, whose answers could change the shape of your problem, or show you that it is not a problem worth working on at all. For example, if none of the other managers are impacted, you may not have a problem at all. If two of them are and two of them aren't, you may have more digging to do. Or if they all agree, you are on to something. You also want to consider how likely each answer is to shut down your problem. For example, if you know that clients are complaining about this, it might not matter what the managers think. You would deem this "not risky."

### Walk your assumptions

With that in mind, take each assumption from Part 6 of your frame and ask three things:

- What are the questions behind this assumption?
- What are the different answers that could change my problem?
...
> **[BUILD]** `delivery_mode: guided_assignment`, `presentation: walkthrough`, no feedback endpoint (no AI on this page). One group task, `layout: table`, `repeat_count: 7`, labels "Assumption 1" to "Assumption 7", rows 4 to 7 marked optional. Export filename `assumptions-walk.docx`. Submission: the text copy into the box as the completion marker; the table pasted into the working book. Fields, each with ask / example / avoid (the garden examples sit in the page prose; the renderer takes one example per field):
>
> - `assumption`, "My assumption" (text). Ask: copy and paste one of the assumptions from Part 6 of your Problem Frame. Example: Managers act on stale ownership for days. Avoid: rewriting it into something safer than what you actually assumed.
> - `questions`, "Questions behind my assumption" (textarea). Ask: turn what you are assuming into a question or questions. Example: How long does it actually take for managers to get updated? Avoid: one question that just restates the assumption with a question mark.
> - `answers`, "Different answers that could change my problem" (textarea). Ask: what possible answers could you get to your question(s)? Example: It takes one day. It takes several days. It takes weeks. Avoid: three answers that all lead to "so I proceed."
> - `risky`, "How risky is this to shutting down my problem?" (textarea). Ask: would any of the answers above lead you to drop the problem, and how likely is that answer? Example: Low risk. The length of time doesn't change whether this is worth pursuing. Avoid: "low risk" with no reason, or marking every row high risk.
>
> Criteria: every assumption from Part 6 has a table; each has at least one question; the answers vary rather than repeating one formula; every risk rating carries a reason. Until Jeremy's table export is ready, the reading fallback is one box: "your tables, one per assumption."
...
>
> DIG INTO YOUR ASSUMPTIONS DOJO - module (v0.1, use with the Dojo Core)
> 
> This module makes you a skeptic who works in my situation. I have walked the assumptions in my Problem Frame: for each one, the questions behind it, the different answers that could change my problem, and how risky it is to shutting the problem down, with a reason. Most of these are unverified, and that is expected; I am not checking any of them this sprint. Your job is to find where my read of my own assumptions is too comfortable, and make me fix it myself. You behave per the Dojo Core; the rounds below are your method.
> 
> == What I bring ==
> My Problem Frame (attached to this project, or pasted), and my assumption tables from the Sprint 2 working book. If I have not pasted the tables, ask for them before Round 1.
> 
> == The rounds ==
> 1. My read (Sensei). Before you look at the tables, ask me which assumption I think is the riskiest and why. Say back what you heard.
> 2. The answers (Challenger). Take the two rows where what I said the answers could change is weakest: where every answer leads to "so I proceed," or where an answer that would end the problem is missing. One row at a time. Quote my words, say what is thin, and ask me to rewrite that cell. Wait for my rewrite before the next row.
> 3. The ratings (Challenger, then Auditor). Find one row I rated low risk too easily, or high risk without a reason, and push: what do I actually know that makes that answer unlikely, or likely? If I defend it with something real, ease off and say so. Then, as Auditor, ask the one question someone who works in this situation would ask before trusting my ratings.
> 4. What I keep (Reflector). Ask me to say, one line each: what you pushed on, what I changed, what I kept and why. Check that the changes are in my words, not yours. Then one line from you: what I gave you that helped, and what you would have wanted earlier.
> 
> == What you do not do ==
> Do not suggest solutions. Do not tell me which assumption to check; there is no check this sprint. Do not add assumptions I did not write; if you think one is missing, ask whether it is in my frame.
> 
> == Start here ==
...
- **dojo-transcript** (response). Prompt: *Paste the complete Dojo transcript, including every CONTINUED chunk, in order.* Criteria: the required header and every turn, in order; every CONTINUED marker and continuation chunk kept; the conversation reaches the module's last round, where you say what you keep.

> **[BUILD]** `dojo_submission: mode: transcript, prompt_version: v1` (the transcript request prompt, not the module), as Sprint 1's Dojo Lab. Single box. Learning goal: "Have AI push on your analysis of your assumptions, and decide what to keep and why." The module text is also the Dojo tab entry for this lab (`dojo-modules-sprint-2.md`). The renderer adds the course-wide confidentiality paragraph at the top and the "Submit the complete transcript" block at the bottom of every transcript Dojo Lab; neither can be placed per page. Both are on the course-wide list.

---

## Your updated assumptions

*Graded item. Position 5. 20 points. One submission.*

This is where your week 1 work gets read. Two parts.

### Part 1: Your updated tables

Paste your **Dig into your assumptions** tables from your **Sprint 2 working book**, every one, as they stand after the Dojo Lab. If the Dojo Lab changed a rating or a reason, paste the changed version; the tables are yours and they are supposed to move.

### Part 2: What the Dojo changed

In your own words: what the Dojo pushed on, what you changed, and what you kept and why. A few sentences is plenty. If you did not use the Dojo Lab, say so and why.

> **Example.** It pushed on "one person's habit, nothing here," and I changed that row: one person's habit can still expose a process that fails whenever accounts move. It pushed on "one day is fast enough to matter" as low risk, and I kept the rating with a better reason, that the length of time does not change whether this is worth pursuing.

### Response tasks

- **the-tables** (response). Prompt: *Your Dig into your assumptions tables, every one, as they stand after the Dojo Lab.* Criteria: every assumption from Part 6 has a table; each has at least one question; the answers vary rather than repeating one formula; every risk rating carries a reason; at least one row rated high risk with the answer that would end the problem, or the reason none is.
- **what-the-dojo-changed** (response). Prompt: *In your own words: what the Dojo pushed on, what you changed, and what you kept and why; or why you did not use it.* Criteria: at least one push named in the participant's words; at least one thing changed or one thing kept with a reason; no bare agreement.

> **[BUILD]** `presentation: reading`, two boxes. Part 1 feeds the-tables, Part 2 what-the-dojo-changed. Rubric to follow the team decision; the risk ratings and their reasons carry most of the points.
...
*Own your progress. Position 7. 0 points. No AI on this one. Live as "What solutions already exist Canvas Walkthrough" (Jeremy's release, 24 September), a three-column grid with Guidance and two example rows; Word download, uploaded in Canvas; the reading version sits unpublished behind it.*
...
[NOTE] Jeremy's line, from the 24 September release; the earlier "copy your table into your Sprint 2 working book" is retired on the walk page.
...
> - `means`, "What using it here would involve" (textarea). Ask: what would change for the better, what it would ask of the people involved, and what I cannot tell yet. Example: The owner would be tracked somewhere people can actually see it. However, I don't know if it can be turned on, and it would require people to fill it out and pay attention to it. Avoid: deciding what it means for your problem before you have looked.
>
...
> **[BUILD]** `dojo_submission: mode: transcript, prompt_version: v1`, as the first Dojo Lab. Single box. Learning goal: "Fill in what exists with AI's help and your own look, and say what each thing means for your problem." The module text is also the Dojo tab entry for this lab (`dojo-modules-sprint-2.md`). Prompts not yet run in a real chat; Leslie's run decides the wording, as with the first Dojo Lab.
...
> **Example.** It added "account transition plans, the kind agencies use." I kept the row, but it will not survive here; two accounts a month cannot carry that much process. It pushed on my "this would solve it" for the CRM owner field, and I changed that cell: the field is not the problem, who makes people fill it in is. I looked at the CRM help page: the field is on every account and can notify a team when the owner changes. Whether ours is switched on, whoever administers the CRM can tell me.

### Response tasks

- **the-table** (response). Prompt: *Your What already exists table, as it stands after the Dojo Lab.* Criteria: at least three rows, questions allowed; nothing in the first column is a solution the participant would build; blanks and "not sure" left as they are; at least one row changed after the Dojo Lab, or the reason none did.
- **what-the-dojo-changed** (response). Prompt: *In your own words: what the Dojo added or pushed on, what you changed, and what you kept and why; where you looked and what you found; or why you did not use it.* Criteria: at least one push or added row named in the participant's words; at least one thing changed or one thing kept with a reason; at least one place looked at outside the chat, with what was found; no bare agreement.

> **[BUILD]** `presentation: reading`, two boxes, the same shape as Your updated assumptions. Learning goal: "Show your What already exists table after the Dojo Lab, and say what changed, what you kept, and what you found when you looked." Questions 1, 4, and 5 of the earlier five-box version are cut (Leslie, 25 September): "does it change your problem" and "how you would now describe it" belong to the Problem Frame item.
...
1. **True or false.** An assumption in your Problem Frame is something you have confirmed with someone who would know. *(False. It is something you wrote as if it were true but do not really know yet. Dig into your assumptions.)*
2. **Multiple choice.** What makes an assumption high risk? (a) You are unsure about it. (b) An answer that could come back would make you drop the problem, and that answer is likely. (c) It is the first one on the list. (d) AI flagged it. *(b. Risk is two things together: an answer that would end the problem, and a real chance of getting that answer. Dig into your assumptions.)*
3. **Multiple choice.** In your What already exists table, the third column, what it means for your problem, is: (a) filled in by AI, since it knows more products than you do; (b) yours to write, because AI cannot know what a thing means for your situation; (c) left blank until Sprint 3; (d) a list of solutions you would build. *(b. AI can help you find out how something works; it cannot know what it means for you. What solutions already exist.)*
4. **Multiple choice.** You paste your own walk into the Dojo before asking it anything. In the 3Cs, that is: (a) Confirmation; (b) Choices; (c) Context; (d) none of them. *(c. Context is giving AI your thinking so it has something to react to. Your work first, then AI. What solutions already exist.)*
5. **True or false.** If your verdict is that the problem is not worth pursuing, you have failed the sprint. *(False. A no is a result. Your runner-up's draft frame is in your Candidate Log. Problem Frame: is it worth pursuing?)*

> **[BUILD]** Five choice tasks, one point each, `presentation: reading`, feedback text as in the parentheses, as Sprint 1's concept check. Learning goal: confirm you can say what an assumption is, what makes one risky, what the table's third column is for, what Context means, and what a no means, before you write your verdict. Moved from position 5 to 9 on 24 September (Leslie and Melisa): a check at the end of week 1 never checks week 2's teaching.

---

## Problem Frame: taking stock of where you are

*Graded item. Position 11. 45 points. One submission. Three boxes.*
...
The work you have done this sprint should result in changes to your understanding of your problem, which means your Problem Frame, which captures your current understanding of your problem, should also change.

Open your Problem Frame document, your Dig into your assumptions tables, and your What already exists table. Use your tables and your thinking in the section above to update your Problem Frame. Based on Sprint 2's work, the likely places you will have updates are at least:
...
2. **Part 3c.** Paste in what you wrote in Part 1 above, trimmed to one or two sentences.
...
Now it's time to consider: does it seem like this problem is worth pursuing, not worth pursuing, or does it depend on something specific? Your reasons are what your walk showed about the risks, what exists, and what you wrote in 3c, weighed by you. You have not tested any of it against a person yet; Sprint 3 does that. So the verdict is on what you know now, and it names the one thing that could still change your mind. Nobody else can make this call, and the activities in this sprint were there to help you make it, not to make it for you.
...
> **Example, a no.** Not worth pursuing as I framed it. My high-risk assumption was that new plot-holders quit because they had no one to ask. When I looked, the garden two streets over pairs new people with returning ones, and half the pairs never speak after the first week; the ones who stay are the ones who talk. What I heard from our own drop-outs was about distance and plot size, not about having nobody to ask. So pairing fixes nothing here, and the problem as I framed it is gone. My runner-up, the sign-up sheet that fills in March and empties by June, is where I go next.

Enter your verdict in the box below.

**If the verdict is no.** That is okay. In fact, that's good. It means all your work this sprint did what it's supposed to do. However, it also means you need to go through the same process with your runner-up problem. It should be much faster this time, since you've already done it once. Open your Candidate Log. Your runner-up's draft frame is in Part C. Copy it into a fresh Problem Frame document and bring it to the same standard, so that you can make the same call about it: dig into its assumptions the way you did in week 1, rate their risk, and start its What already exists table. Sprint 3's conversation becomes its first check. Then write its verdict here, and say why the first problem did not survive. That paragraph is the most useful thing in this submission, and it is graded as such.

### Response tasks

- **what-a-change-would-ask** (response). Prompt: *What fixing this problem would ask of the people involved, and who would have to agree: from the third column of your What already exists table.* Criteria: names at least one thing fixing this would ask of a named person (a step, a habit, time, or a decision); names who would have to agree; draws on the third column of the table, not on a chosen solution.
- **problem-frame** (response). Prompt: *Your Problem Frame, all seven parts, brought up to date: part 6 with its Status column filled, part 3c in, 4a if changed, the date at the top.* Criteria: all seven parts present, in the participant's words; every row of part 6 has an entry in its Status column; every risk rating carries a reason; part 3c is present; at least one assumption is rated high risk and unconfirmed.
- **verdict** (response). Prompt: *Worth pursuing, not worth pursuing, or depends on one named thing, with your reasons. If no: the runner-up's shortened pass and why the first did not survive.* Criteria: one of the three, stated; reasons draw on the risk ratings, what exists, and what a change would ask; names the one thing that could still change the verdict; "depends on" names one thing and who or what could settle it; a no names the runner-up and gives its shortened pass.

> **[BUILD]** `presentation: reading`, three boxes. Learning goal: "Work out what fixing this would ask of people, bring your Problem Frame up to date, and decide whether this problem is worth pursuing." Leslie's docx edits of 25 September applied with five adjustments agreed in chat: "fixing" for "a change"; people named whether or not in 3a; the 3b comparison kept as one sentence; the Status column holds the risk rating with reason; the verdict names the one thing that could change it. Part 7 step cut (duplicates Part 6). A "no" example added from the garden plot.

---

## Sprint 2 Reflection

*Graded item. Position 12. 10 points. Canvas text box, no AI.*

Owned by Melisa's shared format since 24 September (PRs #118 to #120): every sprint's reflection asks the same five questions in the Canvas text box, written without AI: biggest takeaway and biggest challenge; what AI made possible that you could not have done alone; how the work is better because you were involved; what you noticed about how you are working; how the pace and workload are landing. The page is `course1/sprints/sprint-16/sprint-2-reflection-what-the-world-said-back.md` on main, and it is not generated from this draft. Leslie, 25 September: course-wide decisions of Jeremy's and Melisa's stand; anything of that kind is a conversation, not an override.
