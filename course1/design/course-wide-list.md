...
4b. **The transcript block's position.** The renderer places "Submit the complete transcript" (the final Me turn, the request, continue, paste) at the bottom of every transcript Dojo Lab. A page cannot put those steps under its own heading. Proposal for Jeremy: let a page name the section the block renders under, as instruction_section does for tasks. Leslie's wording asks for the block (24 September): drop or reword "send one final Me: turn that states the decisions this activity asks you to make" (plain words, and off where a module covers it, as Sprint 2's do); "Copy and paste the transcript request below"; say what "the required header" is (the one line the request asks the AI to start with: CIS501, Dojo transcript, the date) rather than call it required. The header line in the prompt file uses em dashes, which the course rule bans in authored prose.
