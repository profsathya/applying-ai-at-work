...
- In the Codex in-app browser, each of the six live pages showed one Word button, file-upload submission mode, and the named attachments in its directions. A synthetic answer on each page survived a refresh, then was cleared. Fresh deployed hosted URLs showed the common intro and finish headings on all six. Two long-open instructor tabs briefly retained older cached headings even after ordinary reload; a fresh URL returned the deployed version. The browser's requested mobile viewport override did not change its measured 1280px viewport, so mobile-width evidence comes from the local preview rather than a successful in-app mobile resize.
