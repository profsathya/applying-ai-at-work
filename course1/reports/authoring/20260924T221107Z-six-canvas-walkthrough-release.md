...
- In the Codex in-app browser, each of the six live pages showed one Word button, file-upload submission mode, and the named attachments in its directions. A synthetic answer on each page survived a refresh, then was cleared. Fresh deployed hosted URLs showed the common intro and finish headings on all six. The Codex browser cache retained older embedded headings on 7165 and 7166 even after ordinary reload and reopening the assignment; cache-busted hosted URLs returned the deployed version. GitHub Pages served a 600-second cache lifetime. The browser's requested mobile viewport override did not change its measured 1280px viewport, so mobile-width evidence comes from the local preview rather than a successful in-app mobile resize.
