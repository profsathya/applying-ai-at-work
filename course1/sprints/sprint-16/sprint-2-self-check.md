...
guided_assignment:
  ...
  - id: no-is-a-result
    ...
    prompt: 'True or false: if the verdict at the end of the sprint is that the problem is
      not worth pursuing, you have failed the sprint.'
    criteria:
    ...
    - 'False'
    correct_index: 1
    explanation: False. A no is a result. The runner-up's draft frame is in your Candidate
      Log.
  - id: where-the-frame-lives
    kind: choice
    prompt: Where does your Problem Frame live from this sprint on?
    criteria:
    - Choose the place the frame is kept and brought up to date.
    options:
    - In Part D of the Candidate Log
    - In its own Problem Frame document
    - In the activity boxes
    - In your Dojo
    correct_index: 1
    explanation: The Problem Frame document is where it lives for the rest of the course.
      Each sprint asks you to bring it up to date there.
